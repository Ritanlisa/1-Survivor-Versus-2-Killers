...
long nowTime;
HANDLE hOut;
COORD posStart={0,0},posBegin={20,7},posPause={21,9};
...
{	int playerType=0;
	...
	while(!(KEY_DOWN('A')||KEY_DOWN('D')||KEY_DOWN(VK_RETURN)||KEY_DOWN('F')||KEY_DOWN(VK_RIGHT)||KEY_DOWN(VK_LEFT)));
		...
		if((KEY_DOWN('A')||KEY_DOWN(VK_LEFT))&&ability>0)playerType--;
		if((KEY_DOWN('D')||KEY_DOWN(VK_RIGHT))&&ability<NUM_MAXPLAYERTYPE)playerType++;
		_sleep(150);
	}
}
