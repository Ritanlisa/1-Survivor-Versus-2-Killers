...
long achievementsUnlockedNow=0;
...
void setStep(int posX,int posY,int stepNum)
{
	...
	if(mapG[posX][posY])stepNum+=500;
	if(isAnotherTrapHere(posX,posY,-1))stepNum+=50;
...
bool run()
{
	...
	while(gameOver>0&&killersAlive>0)
	{
		...
		_sleep(500);
		char gotcha;
		while(!(KEY_DOWN(VK_LSHIFT)||KEY_DOWN('W')||KEY_DOWN('A')||KEY_DOWN('S')||KEY_DOWN('D')||KEY_DOWN(VK_SPACE)||KEY_DOWN(VK_LEFT)||KEY_DOWN(VK_RIGHT)||KEY_DOWN(VK_UP)||KEY_DOWN(VK_DOWN)))if(gameOver<=0||killersAlive==0)return gameOver>0&&killersAlive==0;
	...
	for(int i=-NUM_VIEWMAXRADIUS;i<=NUM_VIEWMAXRADIUS;i++)
		for(int j=-NUM_VIEWMAXRADIUS;j<=NUM_VIEWMAXRADIUS;j++)
			...
			if(sqrt(i*i+j*j)<=NUM_VIEWMINRADIUS){watchable[playerPosX+i][playerPosY+j]=true;continue;}
				...
				if(steps[playerPosX+i][playerPosY+j]==abs(i)+abs(j)||steps[playerPosX+i][playerPosY+j]==abs(i)+abs(j)+15||steps[playerPosX+i][playerPosY+j]==abs(i)+abs(j)+20)
				switch(direction)
				{
...
void offlineSurvivor(){
   ...
   long startTimeCount=time(NULL);
	...
	else if(difficulty==3)
	{
		...
		if((achievementsUnlockedNow/2048)%2==0&&time(NULL)-startTimeCount>=360)
		{
	...
	system("cls");
	printf("你输了！\n");
	if((achievementsUnlockedNow/512)%2==0&&difficulty==3&&(time(NULL)-startTimeCount)>=600)
	{
		achievementsUnlockedNow+=512;
	...
	if(result){
	timescore=(log(600)-log(time(NULL)-startTimeCount))*500*(1+difficulty);
	winGameScore=3000*(1+difficulty);
	}else timescore=log(time(NULL)-startTimeCount)*500*(1+difficulty);
