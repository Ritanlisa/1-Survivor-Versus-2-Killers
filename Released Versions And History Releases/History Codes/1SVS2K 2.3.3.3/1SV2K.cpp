...
bool achieveNotHurt=false,achieveBlock=false,achieveTraps=false,start=false,debugging=false,result,watchable[44][44],killersSleep,paused;
...
{	difficulty=-1;
	...
	while(!(KEY_DOWN('A')||KEY_DOWN('D')||KEY_DOWN(VK_RETURN)||KEY_DOWN(VK_LEFT)||KEY_DOWN(VK_RIGHT)||KEY_DOWN('F')));
			...
			if(KEY_DOWN(VK_LEFT)||KEY_DOWN('A'))if(difficulty>-1||(difficulty>-2&&debugEnable))difficulty--;
	...
	for(int x=-21;x<21;x++){
		for(int y=-21;y<21;y++){
			if(difficulty<=2){
				...
				watchable[x+22][y+22]=true;}
			else if(mapG[x+playerPosX][y+playerPosY]&&x*x+y*y<=NUM_VIEWMAXRADIUS*NUM_VIEWMAXRADIUS){
				if(steps[x+playerPosX][y+playerPosY]==abs(x)+abs(y))
				switch(direction)
				{
...
void *showMap(void *id)
{
	...
	for(int i=0;i<22;i++)
	{
		for(int j=0;j<22;j++)
		{
			...
			else if(mapG[i][j]>1||((!(isKillerHere(i,j)||isAnotherTrapHere(i,j,-1)||watchable[i-playerPosX+22][j-playerPosY+22]))&&(!ability))||((!watchable[i-playerPosX+22][j-playerPosY+22])&&ability&&difficulty>0)){
			printf("■");}
			else if(isKillerHere(i,j))
			{printf("K ");
			continue;
	...
	for(int p=0;p<=1+gameOver/100&&p<=5;p++)
	printf("■");
	for(int q=0;q<=(1+gameOver-500)/200;q++)
	printf("■");
	printf("              \n");
	switch(playerType)
	{
